...
def prep_path():
    ...
    atoms_num = 98
    try:
        os.makedirs('seq/seq_v-' + str(atoms_num))
        os.makedirs('mag_cor/poscar_' + str(atoms_num))
        os.makedirs('log/log-' + str(atoms_num))
    except Exception as e:
        print(e)
        # pass

    files = os.listdir('primitive/extend_cell-' + str(atoms_num))
    for ii in range(len(files)):
        primitive_path.append('primitive/extend_cell-%s/POSCAR-v-%s-' % (atoms_num, atoms_num) + str(ii))
        savelog_path.append('log/log-%s/main%s.log' % (atoms_num, ii))
        write_seq_path.append('seq/seq_v-%s/seq_%s' % (atoms_num, ii))
        save_Magcor_path.append('mag_cor/poscar_%s/poscar_%s-%s_Mag_cor.npz' % (atoms_num, atoms_num, ii))
...
def main():
    ...
    ele = SSS
    primitive_path_all, savelog_path_all, write_seq_path_all, save_Magcor_path_all = prep_path()
    for oo in range(len(primitive_path_all)):
        t0 = time.time()
        primitive_path = primitive_path_all[oo]
        savelog_path = savelog_path_all[oo]
        write_seq_path = write_seq_path_all[oo]
        save_Magcor_path = save_Magcor_path_all[oo]
        ...
        for ii,idx in enumerate(order):
            ...
            # set_trace()
            if Mag1.size == 0:
                print('no corresponding struct for %s' % oo)
                break
        # set_trace()
        if Mag1.size != 0:
